...
class SuiteResult:
    ...
    @property
    def passed(self) -> int:
        return sum(1 for t in self.tests if t.status == E2ETestStatus.PASSED)

    @property
    def failed(self) -> int:
        return sum(1 for t in self.tests if t.status == E2ETestStatus.FAILED)
...
class E2ETestRunner:
    ...
    def run_test(self, test: dict[str, Any]) -> TestResult:
        ...
        status = E2ETestStatus.PASSED if validation["passed"] else E2ETestStatus.FAILED
    ...
    def run_suite(self, suite_name: str, suite: dict[str, Any]) -> SuiteResult:
        ...
        for test in suite.get("tests", []):
            ...
            if self.verbose:
                symbol = "✓" if test_result.status == E2ETestStatus.PASSED else "✗"
